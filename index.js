...
app.get('/', (req, res) => {
  let html = `<html>
    <head>
      <title>TOEIC字彙測驗</title>
      <script>window.location = "https://line.me/R/ti/p/@782uiuin";</script>
    </head>
  ...
  if (question_type == 'audio') {
    ...
    for (let i = 0; i < answers.length; i++) {
      ...
      contents.push({
        ...
        "action": {
          "type": "postback",
          "label": String(temp_answer),
          "displayText": String(temp_answer),
          "data": `wid=${w.id}&type=answer&question_type=${question_type}&content=${String(temp_answer)}`
        },
        "style": "secondary",
        "adjustMode": "shrink-to-fit"
      });
...
function createAudioQuestion(event, question_type, current_wid = null) {
  ...
  return {
    ...
    "contents": {
      ...
      "hero": {
        "type": "video",
        "url": `https://toeic.unlink.men/video/${w.id}.mp4`,
        "previewUrl": "https://toeic.unlink.men/audio/cover.png",
        "altContent": {
          "type": "image",
          "url": `https://toeic.unlink.men/audio/cover.png`,
          "flex": 1,
          "size": "full",
          "aspectRatio": "16:9",
          "aspectMode": "cover"
        ...
        "action": {
          "type": "uri",
          "label": "action",
          "uri": `https://toeic.unlink.men/video/${w.id}.mp4`
        },
        "aspectRatio": "16:9"
      },
      "body": {
...
function createAnswers(question_type, wid, total = 3) {
  ...
  for (let counter = 0; counter < new_words.length && array_container.length < total; counter++) {
    ...
    if (new_words[new_gen].word != words[index].word)
...
function playPronounce(event, wid) {
  ...
  getAudioDurationInSeconds(`https://toeic.unlink.men/audio/${w.id}.m4a`).then((duration) => {
    let milliseconds = duration * 1000;
    echo = {
      "type": "audio",
      "originalContentUrl": `https://toeic.unlink.men/audio/${w.id}.m4a`,
